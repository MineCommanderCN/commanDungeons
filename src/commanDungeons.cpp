...
int main() {
	...
	std::ifstream loadtrans(("translate/" + cdl::config_keymap["lang"] + ".lang").c_str());
	if (!loadtrans) {
		std::cout << "FATAL ERROR: Missing file 'translate/" + cdl::config_keymap["lang"] + ".lang'. Please check out your game config or re-install the commanDungeons.\nYour game save will be kept.\n";
		system("pause");
		return 0;
	}
